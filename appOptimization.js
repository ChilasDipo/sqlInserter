...
async function processLineByLine() {
  const fileStream = fs.createReadStream("datafortesting.tsv");
  ...
  for await (const line of rl) {
    ...
    try {
     ...
     // console.log("Arrayet er " + array);
      ...
      request.input("titleType", sql.Int, TitleType.indexOf(array[1])+1);
      ...
      let result = await request.query(
        `INSERT INTO IMDB_Movies([tconst],[titleType],[primaryTitle],[originalTitle],[isAdult],[startYear],[endYear],[runtimeMinutes])VALUES(@tconst,@titleType,@primaryTitle ,@OriginalTitle,@isAdult ,@startYear ,@endYear,@runtimeMinutes)`
      );
      //console.log(result)
